...
drawbar(Monitor *m)
{
	...
	x = 0;
	for (i = 0; i < used; i++) {
        icon = (m->tagset[m->seltags] & 1 << i || occ & 1 << i) 
            ? tagiconfill
            : tagiconoutline;
...
loadxrdb()
{
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        XRDB_LOAD_COLOR("dwm.background", background);
        XRDB_LOAD_COLOR("dwm.foreground", foreground);
        XRDB_LOAD_COLOR("dwm.accent", accent);
        XRDB_LOAD_COLOR("dwm.accentcontrast", accentcontrast);
      }
    }
  }
